...
public abstract class Mapper {
    ...
    static class Keys extends Mapper {
        ...
        public Keys(String keyName) {
            ...
            try {
                ...
                Sponge.getRegistry().getType(Key.class, key).orElseThrow(()->new IllegalArgumentException("There is no such Key: \""+keyName+"\""));
                id = key;
            }
        }
        ...
        Optional<?> map(Optional<?> in) {
            if (in.orElse(null) instanceof ValueContainer<?>) {
                ...
                throw new RuntimeException("Error in 'with'-chain at \""+toString()+"\": Key not supported by input");
            }
        }
    ...
    static class Cause extends Mapper {
        ...
        public Cause(String keyName) {
            key = keyName;
            clz = Utils.tryLoad(keyName).orElseThrow(()->new IllegalArgumentException("No class found for \""+key+"\""));
        }
        Optional<?> map(Optional<?> in) {
            if (in.orElse(null) instanceof Event) {
                ...
                throw new RuntimeException("Error in 'with'-chain at \""+toString()+"\": Not available in cause or input not event");
            }
        }
    ...
    static class Index extends Mapper {
        ...
        public Index(String index) {
            ...
            if (i<0) throw new IllegalArgumentException("Index has to be positive (non-null)");
        }
        Optional<?> map(Optional<?> in) {
            if (in.get() instanceof Array) {
                if (i < Array.getLength(in.get())) return Utils.makeOptional(Array.get(in.get(),i));
                else throw new RuntimeException("Error in 'with'-chain at \""+key+"\": Iterable has not enough elements");
                ...
                throw new RuntimeException("Error in 'with'-chain at \""+key+"\": Iterable element expected");
            }
        }
    ...
    public static Mapper fromString(String def) {
        if (def.isEmpty() || def.equals("#") || def.equals("&")) throw new IllegalArgumentException("Definition required");
        if (def.charAt(0)=='#') {
